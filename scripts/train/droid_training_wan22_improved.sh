#!/bin/bash
# DreamZero DROID Training Script with Wan2.2-TI2V-5B backbone — improved LoRA (rank=32)
#
# Improvements over droid_training_wan22.sh:
#   - lora_rank=32, lora_alpha=32  (8× capacity, ~35M vs 4.4M trainable params)
#   - max_steps=5000               (~10-12h on GB10, vs 100 steps)
#   - learning_rate=5e-5           (appropriate for larger rank)
#   - global_batch_size=4          (gradient_accumulation_steps=4 for better gradients)
#   - save_strategy=steps, save_steps=500  (10 intermediate checkpoints)
#   - TORCH_COMPILE_DISABLE=1      (required on GB10/CUDA 12.1)
#
# Usage:
#   bash scripts/train/droid_training_wan22_improved.sh
#
# Prerequisites:
...
# Disable wandb upload by default; set WANDB_MODE=online to re-enable once `wandb login` is done.
...
OUTPUT_DIR=${OUTPUT_DIR:-"$DREAMZERO_ROOT/checkpoints/dreamzero_droid_wan22_lora_r32"}
...
"${RUN_CMD[@]}" \
    ...
    seed=42 \
    action_head_cfg.config.lora_rank=32 \
    action_head_cfg.config.lora_alpha=32 \
